...
public class PlayerCamera : MonoBehaviour
{
    ...
    // Generate from GetBlockInView(), persist for later use in CmdSetBlock().
    ...
    private void ThirdPerson()
    {
        ...
        if (thirdPerson)
        { // Moves camera backwards for 3rd person view. 
        ...
        { // Moves camera forwards for 2nd person view. 
            float cameraDistance = 5.0f;
            RaycastHit cameraPosFinder;
            Physics.Raycast(transform.localPosition, transform.forward, out cameraPosFinder);
            if (cameraPosFinder.distance > 0.1 && cameraPosFinder.distance < 1 - cameraDistance)
        ...
        { // Standard camera position, you know, on the player's head. 
    ...
    {
        /*if (!localPlayer.jetpackEnabled || Input.GetKey(KeyCode.LeftAlt))
        { // Vertical look axis, rotates the camera only. 
            xRot -= Input.GetAxis("Mouse Y") * Time.deltaTime * sensitivity;
            if (Input.GetKey(KeyCode.UpArrow) == true)
            { // Vertical arrow key look movement. 
        ...
        if (localPlayer.jetpackEnabled)
        { // Vertical look axis, rotates entire player model. 
    ...
    private void HorizontalLook()
    {
        ...
        { // Horizontal look axis, rotates the camera only. 
        ...
        { // Horizontal look axis, rotates entire player model. 
    ...
    {
        // OVRInput.Get(OVRInput.Button.One) // Pad click
        // OVRInput.Get(OVRInput.Button.Two) // Back
        // OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger // Trigger
    ...
    {
        if (!Physics.Raycast(new Ray(localPlayer.movementTransform.position, localPlayer.movementTransform.forward), out hit))
